...
export function GraphTab() {
  return (
    ...
    >
      <aside className="overflow-y-auto border-r border-[var(--hill-rule)] p-3 text-xs">
        <div className="font-semibold uppercase opacity-55">Filters</div>
        <fieldset disabled className="mt-2 flex flex-col gap-1">
          <legend className="opacity-55">Layer</legend>
          {LAYERS.map((l) => (
          ...
          <legend className="mt-3 opacity-55">Claim type</legend>
          {CLAIM_TYPES.map((c) => (
      ...
      <section className="flex flex-col">
        <div className="flex items-center gap-2 border-b border-[var(--hill-rule)] px-3 py-2 text-xs">
          ...
          <label className="flex items-center gap-2">
            layout
            <select disabled className="border border-[var(--hill-rule)] px-2 py-1">
              <option>force</option>
            </select>
          </label>
          <button disabled className="border border-[var(--hill-rule)] px-2 py-1">
            reset
          </button>
        </div>
        <div className="grid flex-1 place-items-center">
          <div className="border border-dashed border-[var(--hill-rule)] p-6 text-center text-sm opacity-55">
      ...
      <aside className="overflow-y-auto border-l border-[var(--hill-rule)] p-3 text-xs">
        <div className="font-semibold uppercase opacity-55">Selected node</div>
        <div className="mt-2 opacity-55">no selection</div>
      </aside>
    </div>
  );
}
